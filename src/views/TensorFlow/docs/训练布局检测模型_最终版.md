...
这导致模型陷入了严重的混淆，完全无法建立起一个可靠的识别规则。我们为此付出了巨大的努力，反复尝试了双类别、五类别、乃至最终的三类别数据标注方案。尽管每一次的训练报告（`results.png`）都给了我们喜人的高分 ✅，但在前端用同一张图片进行实地测试时，结果却总是惨不忍睹 😭。
