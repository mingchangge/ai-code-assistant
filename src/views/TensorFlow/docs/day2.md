# 使用预训练 CNN 模型，对手写数字识别（MNIST）

在本节中，我们将使用 TensorFlow.js 构建一个简单的手写数字识别应用。我们将使用预训练的卷积神经网络（CNN）模型来识别用户在画布上绘制的数字。

## 准备工作

1. **安装 TensorFlow.js**：确保你已经在项目中安装了 TensorFlow.js。如果还没有安装，可以使用以下命令进行安装：
...
2. **引入预训练的 MNIST 模型**：
   1. 使用 python 将 MNIST 模型数据转换为 json 格式（[MachineLearning-python](https://github.com/mingchangge/MachineLearning-python)/convert_mnist_to_json.py）
   2. 将转换后的 json 文件放入到 node 项目（[mnist-tfjs-node](https://github.com/mingchangge/mnist-tfjs-node)）中构建并训练模型生成模型文件 `model.json`和权重文件 `weights.bin`
   3. 将生成的 `model.json` 和 `weights.bin` 文件放入到项目的 `public/models/mnist-cnn` 目录下

💡 **<font color='red'>重要提示： </font>** 使用稳定版本的node和python，避免兼容性问题！

3. **创建组件（src/views/TensorFlow/components/DigitRecognizer.tsx）实现手写数字识别功能**：
